...
public class TSPData {
	...
	static double[][] cityPosition;
	static final int SPECIES_NUM=500; //种群数
	...
	public static void LoadData(String path) {
		ArrayList<ArrayList<String>> citys = Util.LoadData(path," ");
		cityPosition = new double[citys.size()][2];
		for(int i=0;i<citys.size();i++) {
			cityPosition[i][0] = Double.parseDouble(citys.get(i).get(1));
			cityPosition[i][1] = Double.parseDouble(citys.get(i).get(2));
		}
